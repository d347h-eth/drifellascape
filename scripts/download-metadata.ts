...
const BASE_URL = 'https://lime-given-booby-462.mypinata.cloud/ipfs/bafybeihvgvggho4jvvmikybbil4ylylabztxhyn6sczi5w2kvzejcozu2a/{id}';
const START_ID = 0;
...
async function main() {
  ...
  for (let id = START_ID; id <= END_ID; id++) {
    ...
    process.stdout.write(`Downloading ${id} -> ${path.relative(process.cwd(), outPath)}\n`);
    try {
      ...
      try {
        ...
        await logError(`Non-JSON response for id=${id}: ${(e as Error).message}`);
      }
      await fs.writeFile(outPath, body, 'utf8');
      successCount += 1;
    } catch (err) {
      await logError(`Failed to download id=${id} after ${MAX_RETRIES} attempts: ${String((err as Error)?.message || err)}`);
      failCount += 1;
    }
  }

  process.stdout.write(`Done. Success: ${successCount}, Failed: ${failCount}\n`);
}
