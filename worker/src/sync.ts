...
export async function syncListings(
    ...
    try {
        ...
        if (activeId === 0) {
            throw new Error(
                "Invariant violated: expected a single active row in listing_versions",
            );
        }
       ...
       VALUES (unixepoch('now'), ?, 0)`,
        ...
        const newVersionId = (
            db.raw.prepare("SELECT last_insert_rowid() AS id").get() as {
                id: number;
            }
        ).id;
       ...
       FROM temp_listings`,
        ...
        if ((res.changes | 0) !== total) {
            ...
            db.raw
                ...
                .run(newVersionId);
            db.raw
                .prepare("DELETE FROM listing_versions WHERE id = ?")
                .run(newVersionId);
            throw new Error(
                `Snapshot insert mismatch: expected ${total}, inserted ${res.changes}`,
            );
        }
