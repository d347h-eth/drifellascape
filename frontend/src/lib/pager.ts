...
export type PageResult = { items: Row[]; baseOffset: number; total: number; versionId: number | null };
...
export async function loadInitialPage(params: {
  ...
  return { items: data.items ?? [], baseOffset: Number(data.offset || 0), total: Number(data.total || 0), versionId: (data as any).versionId ?? null };
}
